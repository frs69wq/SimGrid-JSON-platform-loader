...
static constexpr const char* S_INST_TO_DTN        = "inst_to_dtn";
static constexpr const char* S_SLAC_TO_NERSC               = "slac_to_nersc";
static constexpr const char* S_CORI_TO_DTN   = "cori_to_dtn";
static constexpr const char* F_SLAC = "slac-scratch:slac-scratch-fs:/slac-scratch/instrument.out";
static constexpr const char* F_NERSC = "nersc-scratch:nersc-scratch-fs:/nersc-scratch/analysis.out";
static constexpr const char* F_SLAC_TO_NERSC = "slac_to_nersc_out";

// ─── Helper: open + define variable for publishing ────────────────────────────

static std::pair<std::shared_ptr<dtlmod::Engine>, std::shared_ptr<dtlmod::Variable>>
open_pub(std::shared_ptr<dtlmod::DTL> dtl, const char* stream_name, const char* file_name)
{
  const auto& stream = dtl->get_stream_by_name(stream_name).value_or(nullptr);
  if (not stream)
    xbt_die("Unknown stream '%s'", stream_name);
  auto engine = stream->open(file_name, dtlmod::Stream::Mode::Publish);
...
static std::pair<std::shared_ptr<dtlmod::Engine>, std::shared_ptr<dtlmod::Variable>>
open_sub(std::shared_ptr<dtlmod::DTL> dtl, const char* stream_name, const char* file_name)
{
  const auto& stream = dtl->get_stream_by_name(stream_name).value_or(nullptr);
  if (not stream)
    xbt_die("Unknown stream '%s'", stream_name);
  auto engine = stream->open(file_name, dtlmod::Stream::Mode::Subscribe);
...
static void act_instrument(int nsteps)
{
  ...
  auto [engine, var] = open_pub(dtl, S_INST_TO_DTN, F_SLAC);

  for (int s = 1; s <= nsteps; s++)
    do_put(engine, var, s, S_INST_TO_DTN);
...
static void act_slac_dtn(int nsteps)
{
  ...
  auto [sub_e, sub_v] = open_sub(dtl, S_INST_TO_DTN, F_SLAC);
  auto [pub_e, pub_v] = open_pub(dtl, S_SLAC_TO_NERSC, F_SLAC_TO_NERSC);

  for (int s = 1; s <= nsteps; s++) {
    do_get(sub_e, sub_v, s, S_INST_TO_DTN);
    do_put(pub_e, pub_v, s, S_SLAC_TO_NERSC);
  }
...
static void act_nersc_dtn(int nsteps)
{
  auto dtl            = dtlmod::DTL::connect();
  simgrid::s4u::this_actor::sleep_for(4);
  auto [sub_e, sub_v] = open_sub(dtl, S_SLAC_TO_NERSC, F_SLAC_TO_NERSC);
  auto [pub_e, pub_v] = open_pub(dtl, S_CORI_TO_DTN, F_NERSC);

  for (int s = 1; s <= nsteps; s++) {
    do_get(sub_e, sub_v, s, S_SLAC_TO_NERSC);
    do_put(pub_e, pub_v, s, S_CORI_TO_DTN);
  }
...
{
  auto dtl           = dtlmod::DTL::connect();
  simgrid::s4u::this_actor::sleep_for(6);
  // auto [engine, var] = open_sub(dtl, S_SCRATCH_TO_CORI);
  auto [engine, var] = open_sub(dtl, S_CORI_TO_DTN, F_NERSC);

  for (int s = 1; s <= nsteps; s++)
    // do_get(engine, var, s, S_SCRATCH_TO_CORI);
    do_get(engine, var, s, S_CORI_TO_DTN);
...
//   e.host_by_name("ffb_server"      )->add_actor("ffb",        [nsteps]() { act_ffb(nsteps);        });
  e.host_by_name("node-0.slac-dtn")->add_actor("slac_dtn",   [nsteps]() { act_slac_dtn(nsteps);   });
  e.host_by_name("node-0.nersc-dtn" )->add_actor("nersc_dtn",   [nsteps]() { act_nersc_dtn(nsteps);   });
